...
async def my_orders_history(call: types.CallbackQuery):
    ...
    for order in orders:
        ...
        address = order['address_text'] or "Olib ketish"
        payment = order['payment_type'] or "Noma'lum"
        ...
        for item in items:
            ...
            p_total = "{:,.0f}".format(item['price'] * item['quantity']).replace(",", " ")
            
            items_text += (
                f"<b>{item['product_name']}</b>\n"
                f" <code>{item['quantity']}</code> ✖️ {p_price} = <b>{p_total}</b>\n"
            )
